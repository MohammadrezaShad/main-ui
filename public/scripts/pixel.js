...
const PixelJS = {
  ...
  existsInArray(item, arrayToSearch) {
    let i = arrayToSearch.length;
    while (i--) {
  ...
  proxy(callback, context, additionalArguments) {
...
PixelJS.Engine = function () {
  ...
  document.onkeydown = function (e) {
    ...
    const {keyCode} = e;
    const listeners = self._events.keydown;
  ...
  document.onkeyup = function (e) {
    e = e || event; // Small hack to get the event args in IE
    const {keyCode} = e;
    const listeners = self._events.keyup;
    if (listeners.length > 0) {
      e.preventDefault();
      listeners.forEach(function (listener) {
        listener(keyCode);
...
PixelJS.Engine.prototype._checkForCollissions = function () {
  for (let keyIndex = 0; keyIndex < this._layerKeys.length; keyIndex++) {
    // Check for collisions within the layer's own collidables
    const collidables = this._layers[this._layerKeys[keyIndex]]._collidables;
    ...
    for (let k = 0; k < this._layerKeys.length; k++) {
      if (k !== keyIndex) {
        const otherCollidables = this._layers[this._layerKeys[k]]._collidables;
        for (var s = 0; s < collidables.length; s++) {
...
PixelJS.Engine.prototype._unregisterGameLoopCallback = function (callback) {
  for (let i = this._gameLoopCallbacks.length - 1; i >= 0; i--) {
...
PixelJS.Engine.prototype._toggleFPSLayer = function () {
  if (this._displayFPS) {
    if (this._layers.__pixeljs_performanceLayer === undefined) {
      const layer = this.createLayer('__pixeljs_performanceLayer');
      const counter = new PixelJS.FPSCounter(layer);
  ...
  } else if (this._layers.__pixeljs_performanceLayer !== undefined) {
...
PixelJS.Engine.prototype.createLayer = function (name) {
  const layer = new PixelJS.Layer(this);
...
PixelJS.Engine.prototype.init = function (info) {
  ...
  this._inputLayer.onmousemove = function (e) {
    const listeners = self._events.mousemove;
    if (listeners.length > 0) {
      const point = self._inputLayer.relMouseCoords(e);
      listeners.forEach(function (listener) {
        listener(point);
      });
    }
  ...
  this._inputLayer.onmouseup = function (e) {
    const listeners = self._events.mouseup;
    if (listeners.length > 0) {
      // The middle button is usually 1 but should be dispatched
      // as 4 to allow bitwise operations in the future.
      const button = e.button == 1 ? 4 : e.button == 0 ? 1 : 2;
...
PixelJS.Engine.prototype.loadScene = function (callback) {
  let loading = this._layerKeys.length;
  for (let k = 0; k < this._layerKeys.length; k++) {
...
PixelJS.Engine.prototype.loadSounds = function (callback) {
  let loading = this._soundKeys.length;
  if (loading === 0) {
    callback();
  } else {
    for (let k = 0; k < this._soundKeys.length; k++) {
      const key = this._soundKeys[k];
...
PixelJS.Engine.prototype.off = function (event, callback) {
  for (let i = this._events[event.toLowerCase()].length - 1; i >= 0; i--) {
...
PixelJS.Entity.prototype._setIsClickable = function (val) {
  ...
  if (val) {
    ...
    if (!this._isDraggable) {
      this._mousedownHook = function (p, b) {
        self._onMouseDown(p, b);
      };
      this._mouseupHook = function (p, b) {
        self._onMouseUp(p, b);
      };
...
PixelJS.Entity.prototype.collidesWith = function (entity) {
  return (
    this.pos.x + this.size.width > entity.pos.x &&
...
PixelJS.Entity.prototype.fadeTo = function (opacity, duration, callback) {
  ...
  const animationSpeed = (this.opacity - opacity) / duration;
  const increasingOpacity = this.opacity < opacity;
  const self = this;
  ...
  this._animateOpacity = function (elapsedTime, dt) {
    dt *= 1000; // Convert into milliseconds from fractional seconds.
    ...
    if (
      (self.opacity >= opacity && increasingOpacity) ||
      (self.opacity <= opacity && !increasingOpacity)
    ) {
...
PixelJS.Entity.prototype.moveTo = function (x, y, duration, callback) {
  ...
  const velocityX = (this.pos.x - x) / duration;
  const velocityY = (this.pos.y - y) / duration;
  const targetIsToTheLeft = x < this.pos.x;
  const targetIsAbove = y < this.pos.y;
  const self = this;
  ...
  this._animateMovement = function (elapsedTime, dt) {
    ...
    if (
      ((targetIsToTheLeft && self.pos.x <= x) || (!targetIsToTheLeft && self.pos.x >= x)) &&
      ((targetIsAbove && self.pos.y <= y) || (!targetIsAbove && self.pos.y >= y))
    ) {
      self.pos.x = x;
      self.pos.y = y;
      self.layer.engine._unregisterGameLoopCallback(self._animateMovement);
...
PixelJS.Layer.prototype._insertIntoDom = function () {
  const {container} = this.engine.scene;
...
PixelJS.Layer.prototype._registerDraggable = function (draggable) {
  if (this._draggables.length == 0) {
    ...
    this.engine.on('mousemove', function (point) {
      for (let i = 0; i < self._draggables.length; i++) {
...
PixelJS.Layer.prototype._unregisterDraggable = function (draggable) {
  for (let i = this._draggables.length - 1; i >= 0; i--) {
...
PixelJS.Layer.prototype.createEntity = function () {
  const entity = new PixelJS.Entity(this);
...
PixelJS.Layer.prototype.load = function (callback) {
  let loading = this._components.length;
  if (loading === 0) {
    callback();
  } else {
    for (let i = 0; i < this._components.length; i++) {
...
PixelJS.Layer.prototype.unregisterCollidable = function (collidable) {
  for (let i = this._collidables.length - 1; i >= 0; i--) {
...
Object.defineProperty(PixelJS.Layer.prototype, 'zIndex', {
  get() {
    return this._canvas.style.zIndex;
  },
  set(val) {
    this._canvas.style.zIndex = val;
  },
  configurable: false,
  enumerable: false,
});
...
Object.defineProperty(PixelJS.Sound.prototype, 'playbackRate', {
  get() {
    return this._element.defaultPlaybackRate;
  },
  set(val) {
    this._element.defaultPlaybackRate = Math.max(val, 0.1);
  },
  configurable: false,
  enumerable: true,
});
...
PixelJS.AnimatedSprite.prototype.draw = function (entity) {
  if (this.loaded) {
      ...
      if (!isNaN(entity.layer.engine._deltaTime)) {
        ...
        if (this._frameTickCount >= this.speed) {
          ...
          if (this.resetFrame < 1) {
            this._currentFrame =
              this._currentFrame == this.spriteSheet.frameCount - 1 ? 0 : this._currentFrame + 1;
          } else {
    ...
    entity.layer.drawImage(
      this.spriteSheet._frameImages[this.row][this._currentFrame],
      entity.pos.x,
      entity.pos.y,
      undefined,
      entity.opacity,
...
PixelJS.SpriteSheet.prototype._getFrameData = function (row, frame, transparencyKey) {
  ...
  const pixels = ctx.getImageData(0, 0, this._frameSize.width, this._frameSize.height);
...
PixelJS.SpriteSheet.prototype.load = function (info, callback) {
  const self = this;
  const rows = info.rows != undefined ? info.rows : 1;
  this._rowCount = rows;
